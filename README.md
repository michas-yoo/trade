...
Т-банк не предоставляет токены для коммерческого использования, поэтому отправка сообщений ограничена одним чатом – с
самим собой. Для настройки чата нужно создать ТГ-бота, получить его токен, получить айди чата с ботом и ввести данные в
`.env`-файл.

В текущем проекте настроен CRON – проверка и отправка сообщений происходят каждые 3 часа с 10 до 19.
